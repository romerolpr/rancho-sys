...
$stt = $db->prepare("SELECT * FROM " .TB_RESP. " WHERE datasheet=\"".$_SESSION["objfile"]["name"]."\"");
$stt->execute();
$fromDbComplete = $stt->fetchAll(PDO::FETCH_ASSOC);

sortByColumns($fromDbComplete);
	...
	// break;
		...
		foreach ($ObjDecoded["refc"] as $keyRefc => $valueRefc):
			...
			if ($Today == $valueRefc[1] || isset($get["exb_all"])):
				$sheetHeader .= "<td><span class=\"title\">[" . $diasemana[$diasemana_numero] . "] " .  $valueRefc[1] . "".($Render->getFilter() ? "</span></div>" : null)."</td>";
			endif;
				...
				if (is_array($arranchamento)):
					...
					if (empty($arranchamento[0])):
						...
						$sheetBody .= "<td data-hash-id=\"".$value["hash"]."\" data-date=\"".$valueRefc[1]."\">" . 
							...
							(isset($arranchamento[0]) ? "<div><label for=\"1".$idItem."\">" .resizeString($arranchamento[0], 8). "</label><input ".($Checked[0] !== false ? "checked" : null)." data-date=\"".$valueRefc[1]."\" class=\"input_checked\" type=\"checkbox\" name=\"check_refc\" value=\"".resizeString($arranchamento[0], 8)."\" id=\"1".$idItem."\"></div>" : "</div>") . 
